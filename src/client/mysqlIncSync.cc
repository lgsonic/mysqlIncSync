...
{
	char szBuf[64];
	snprintf(szBuf, 64, "%llu", value);
	return szBuf;
}

struct st_Config
{
  std::string binlog_index;
  std::string binlog_last_file;
...
struct st_RowValue
{
  ...
  std::string to_json_string()
  {
    ...
    jsonwriter.String("command").String(command.c_str(), command.size());
    jsonwriter.String("table").String(table.c_str(), table.size());
    jsonwriter.String("timestamp").Int64(time(0));
    uint nFieldnum = new_value_list.size() > 0 ? new_value_list.size(): old_value_list.size();
    jsonwriter.String("fieldnum").Uint(nFieldnum);

    if (new_value_list.size() > 0)
    {
      jsonwriter.String("newrow");
      jsonwriter.StartObject();
      for (size_t i = 0; i < new_value_list.size(); ++i)
      {
        jsonwriter.String(to_string((uint32)i).c_str()).String(new_value_list[i].c_str(), new_value_list[i].size());
      }
      jsonwriter.EndObject();
    }

    if (old_value_list.size() > 0)
    {
      jsonwriter.String("oldrow");
      jsonwriter.StartObject();
      for (size_t i = 0; i < old_value_list.size(); ++i)
      {
        jsonwriter.String(to_string((uint32)i).c_str()).String(old_value_list[i].c_str(), old_value_list[i].size());
      }
      jsonwriter.EndObject();
    }
...
static Exit_status dump_local_log_entries(PRINT_EVENT_INFO *print_event_info,
                                          const char* logname, my_off_t logstartpos, my_off_t & logendpos);
...
static Exit_status dump_log_entries(const char* logname, my_off_t logstartpos, my_off_t & logendpos)
{
  ...
  */
  ...
  rc= (remote_opt ? dump_remote_log_entries(&print_event_info, logname) :
       dump_local_log_entries(&print_event_info, logname, logstartpos, logendpos));
                                          ...
                                          const char* logname, my_off_t logstartpos, my_off_t & logendpos)
{
  ...
  if (logname && strcmp(logname, "-") != 0)
  {
    ...
    if (logstartpos > start_position_mot)
    {
      my_b_seek(file, (my_off_t)logstartpos);
    }
#if 1
    if (!glob_description_event)	// check_header only when glob_description_event is null
#endif
...
static Exit_status get_log_name_list(const char* logindexname, std::vector<std::string> & lognamelist)
{
  std::ifstream ifs(logindexname, std::ios::in);
  if (!ifs.good())
  {
    return ERROR_STOP;
  }
  ...
  while (!ifs.eof())
  {
    char szLine[nConfigBufSize];
    if (ifs.getline(szLine, nConfigBufSize).good())
    {
      lognamelist.push_back(szLine);
    }
    else
    {
      break;
...
static Exit_status read_config()
{
  ...
  inireader.getValue("config", "binlog_index", szBuf, &nSize);
  stConfigValue.binlog_index = szBuf;
  
  nSize = nConfigBufSize;
  memset(szBuf, 0, nConfigBufSize);
  inireader.getValue("config", "binlog_last_file", szBuf, &nSize);
  stConfigValue.binlog_last_file = szBuf;
  
  nSize = nConfigBufSize;
  memset(szBuf, 0, nConfigBufSize);
  inireader.getValue("config", "binlog_last_offset", szBuf, &nSize);
  stConfigValue.binlog_last_offset = atoll(szBuf);
...
static Exit_status write_config()
{
  ...
  iniwriter.SetValue("config", "binlog_index", stConfigValue.binlog_index.c_str());
  iniwriter.SetValue("config", "binlog_last_file", stConfigValue.binlog_last_file.c_str());
  char szBuf[64] = {0};
  snprintf(szBuf, 64, "%llu", stConfigValue.binlog_last_offset);
...
static bool check_table_valid(const char * table)
{
  ...
  try
  {
    static const std::tr1::regex pattern(stConfigValue.tables.c_str());
    return std::tr1::regex_match(table, pattern);
  }
  catch (...)
  {
    return true;
...
//    REG_LOG_PROCESSOR(display_log);
    REG_LOG_PROCESSOR(write_log_to_redis);
  }
protected:
...
#define MYSQLINCSYNC_VERSION "1.0.8"
  ...
  */
  ...
  while (1)
  {
    ...
    for (size_t i = nLogStartIndex; i < LogNameList.size(); )
    {
      ...
      std::string strCurName = LogNameList[i];
      my_off_t nCurPos = 0;
      
      if (strCurName == stConfigValue.binlog_last_file)
      {
        nCurPos = stConfigValue.binlog_last_offset;
        bStart = true;
      }
      ...
      if (dump_log_entries(strCurName.c_str(), nCurPos, stConfigValue.binlog_last_offset) != OK_CONTINUE)
      {
        LOG(ERROR)("dump_log_entries failed");
        exit(1);
      }

      stConfigValue.binlog_last_file = strCurName;
