...
log_event_print_value(IO_CACHE *file, const uchar *ptr,
                      uint type, uint meta,
                      char *typestr, size_t typestr_length, std::string & strValue)
{
  uint32 length= 0;
  ...
  case MYSQL_TYPE_TINY:
    {
      ...
      strValue = to_string((int32) (signed char) *ptr);
      return 1;
    }
  ...
  case MYSQL_TYPE_BIT:
    {
      ...
      my_snprintf(typestr, typestr_length, "BIT(%d)", nbits);
      {
        ...
        {
          i64 = i64*256 + (uint) (unsigned char) ptr[length-i-1];
        }
        strValue = to_string(i64);
      }
      return length;
    }
  ...
  case MYSQL_TYPE_SET:
    ...
    my_snprintf(typestr, typestr_length, "SET(%d bytes)", meta & 0xFF);
    {
      ...
      {
        i64 = i64*256 + (uint) (unsigned char) ptr[nLen-i-1];
      }
      strValue = to_string(i64);
    }
    return meta & 0xFF;
  ...
  case MYSQL_TYPE_BLOB:
    ...
    case 1:
      ...
      strValue.insert(strValue.end(), ptr + 1, ptr + 1 + length);
      return length + 1;
    case 2:
      length= uint2korr(ptr);
#ifdef PRINT_VERBOSE
      ...
      strValue.insert(strValue.end(), ptr + 2, ptr + 2 + length);
      return length + 2;
    case 3:
      ...
      strValue.insert(strValue.end(), ptr + 3, ptr + 3 + length);
      return length + 3;
    case 4:
      ...
      strValue.insert(strValue.end(), ptr + 4, ptr + 4 + length);
      return length + 4;
    default:
  ...
  case MYSQL_TYPE_VAR_STRING:
  {
    ...
    {
      nLen = *ptr;
      strValue.insert(strValue.end(), ptr + 1, ptr + 1 + nLen);
      nLen += 1;
    }
    else
    {
      nLen = uint2korr(ptr);
      strValue.insert(strValue.end(), ptr + 2, ptr + 2 + nLen);
      nLen += 2;
    }
#ifdef PRINT_VERBOSE
    my_b_write_quoted_with_length(file, ptr, length);
#endif
    return nLen;
  }
...
Rows_log_event::print_verbose_one_row(IO_CACHE *file, table_def *td,
                                      ...
                                      const uchar *value, const uchar *prefix, std::vector<std::string> & value_list)
{
  ...
  for (size_t i= 0; i < td->size(); i ++)
  {
    ...
    else
    {
      ...
      size_t size= log_event_print_value(file, value,
                                         ...
                                         typestr, sizeof(typestr), strValue);
      if (!size)
        return 0;
#if 1
      value_list.push_back(strValue);
#endif
      value+= size;
    }
#if 0
                                   ...
                                   PRINT_EVENT_INFO *print_event_info)
{
...
#if 1
    LOG(ERROR)("impossible row type: %d", type_code);
    return;
#endif
  }
  ...
  {
    st_RowValue row_value;
    row_value.command = sql_command;
    row_value.table = (std::string)map->get_db_name() + "." + map->get_table_name();
    ...
    case WRITE_ROWS_EVENT:
    {
      ...
      if (!(length= print_verbose_one_row(file, td, print_event_info,
                                    &m_cols, value,
                                    (const uchar*) sql_clause1, row_value.new_value_list)))
        goto end;
      value+= length;
      break;
    }
    default:
    {
      ...
      if (!(length= print_verbose_one_row(file, td, print_event_info,
                                    &m_cols, value,
                                    (const uchar*) sql_clause1, row_value.old_value_list)))
        goto end;
      value+= length;
      break;
    }
    ...
    if (sql_clause2)
    {
      if (!(length= print_verbose_one_row(file, td, print_event_info,
                                      &m_cols_ai, value,
                                      (const uchar*) sql_clause2, row_value.new_value_list)))
        goto end;
      value+= length;
    }
    ...
    {
      std::string strRowValue = row_value.to_json_string();
...
#if 0
printf("11============================  m_table_id: %lu  m_flags: %d  m_width: %lu  data_size: %lu  common_header_len: %d   post_header_len: %d \n",
                     m_table_id, m_flags, m_width, (ulong) (ptr_after_width - (const uchar *) buf), common_header_len, post_header_len);
#endif
  ...
  if (event_type == UPDATE_ROWS_EVENT)
  {
    DBUG_PRINT("debug", ("Reading from %p", ptr_after_width));
#if 0
printf("22============================  m_table_id: %lu  m_flags: %d  m_width: %lu  data_size: %lu\n",
                     m_table_id, m_flags, m_width, (ulong) (ptr_after_width - (const uchar *) buf));
#endif
    /* if bitmap_init fails, caught in is_valid() */
...
#if 0
  printf("33============================  m_table_id: %lu  m_flags: %d  m_width: %lu  data_size: %lu\n",
                     m_table_id, m_flags, m_width, (ulong) (ulong) (ptr_after_width - (const uchar *) buf));
#endif
